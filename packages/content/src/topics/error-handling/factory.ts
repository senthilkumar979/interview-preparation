...
import { errorHandlingLevels } from "./levels";

export interface ErrorHandlingTopicDraft {
  slug: string;
  title: string;
  order: number;
  summary: string;
...
export function errorHandlingTopic(draft: ErrorHandlingTopicDraft): Topic {
  return {
    slug: draft.slug,
    title: draft.title,
    technologySlug: "error-handling",
    module: "Error handling",
    order: draft.order,
    summary: draft.summary,
    prerequisites: draft.prerequisites ?? [],
    related: draft.related ?? [],
    levels: errorHandlingLevels,
    ...
    codeExample: {
      language: draft.language ?? "typescript",
