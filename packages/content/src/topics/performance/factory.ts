...
import { performanceLevels } from "./levels";

export interface PerformanceTopicDraft {
  slug: string;
  title: string;
  order: number;
  summary: string;
...
export function performanceTopic(draft: PerformanceTopicDraft): Topic {
  return {
    slug: draft.slug,
    title: draft.title,
    technologySlug: "performance",
    module: "Performance",
    order: draft.order,
